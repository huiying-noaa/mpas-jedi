...
subroutine add_incr(self, increment)
   ...
   if (self%geom%nCells==increment%geom%nCells .and. self%geom%nVertLevels==increment%geom%nVertLevels) then
      ...
      if (self%has('air_temperature_at_2m') .and. associated(ptrr2_dt)) then
         call self%get('air_temperature_at_2m', ptrr1_2mt)
         ptrr1_2mt(1:ngrid)  = ptrr1_2mt(1:ngrid) + ptrr2_dt(1,1:ngrid)
      endif
      if (self%has('water_vapor_mixing_ratio_wrt_moist_air_at_2m') .and. &
          associated(ptrr2_dsh)) then
